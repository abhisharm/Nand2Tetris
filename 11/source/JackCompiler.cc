...
using AbhishekJackCompiler::CompilationEngine;
using AbhishekJackCompiler::VMWriter;

JackCompiler::JackCompiler(vector<string> args) : args_(args) {}

JackCompiler::ArgumentDescriptor JackCompiler::GetArgumentType(string arg) {
  ...
  if (!std::regex_match(arg, sm, fname_regex)) {
    ArgumentDescriptor argument_descriptor = {
      .argument_type = JackCompiler::ArgumentType::UNMATCHED,
    };
  ...
  if (suffix.size() != 0) {
    ...
    JackCompiler::ArgumentDescriptor argument_descriptor = {
      .argument_type = JackCompiler::ArgumentType::FILENAME,
      .path = path,
      .name = prefix,
    };
    return argument_descriptor;
...
void JackCompiler::ConstructSimplifiedOutputForFile(
  JackCompiler::ArgumentDescriptor argument_descriptor) {
...
void JackCompiler::ConstructOutputForFile(
  JackCompiler::ArgumentDescriptor argument_descriptor) {
  // create proper input and output streams
  string input = argument_descriptor.path + argument_descriptor.name + ".jack";
  cout << "reading from file: " << input << "\n";
  std::ifstream i_stream { input };
  string output = argument_descriptor.path + argument_descriptor.name + ".vm";
  cout << "writing to file: " << output << "\n";
  // create the tokenizer
  std::ofstream o_stream { output };
  JackTokenizer tokenizer(&i_stream);
  ...
  CompilationEngine compilation_engine(&tokenizer, &o_stream, "testfile");
...
void JackCompiler::ConstructOutputForDirectory(
  ArgumentDescriptor argument_descriptor) {
  ...
  for (auto const& dir_entry
    ...
    if (argument_descriptor.argument_type == JackCompiler::ArgumentType::FILENAME) {
      ConstructOutputForFile(argument_descriptor);
    }
  }
}

void JackCompiler::ConstructOutputForAllArgs() {
  for (auto arg : args_) {
    ArgumentDescriptor argument_descriptor = GetArgumentType(arg);
    switch (argument_descriptor.argument_type) {
      case JackCompiler::ArgumentType::FILENAME:
        ConstructOutputForFile(argument_descriptor);
        break;
      case JackCompiler::ArgumentType::DIRNAME:
        ConstructOutputForDirectory(argument_descriptor);
        break;
      case JackCompiler::ArgumentType::UNMATCHED:
        break;
    }
  }
  return;
...
int main(int argc, char** argv) {
  ...
  AbhishekJackCompiler::JackCompiler analyzer(args);
