...
namespace Translator {
  ...
  void Parser::advance() {
    ...
    std::regex pat (R"(([\w-]+)\s*([\w.]*)\s*(\d*)\s*(\/{2}[\S\s]*)?)");
    ...
    if (command == "add" | command == "sub" | command == "neg" |
        ...
        string function_name (sm[2]);
        arg1_ = function_name;
        string nvars_string (sm[3]);
        arg2_ = atoi(nvars_string.c_str());
        ...
        string function_name (sm[2]);
        arg1_ = function_name;
        string nargs_string (sm[3]);
        arg2_ = atoi(nargs_string.c_str());
    } else {
